...
export class FirstTransactionsPage {
  ...
  static open() : FirstTransactionsPage {
    ...
    cy.wait('@transactionsList').its('response.statusCode').should('equal', 200, 'Check transactions list loaded');
...
class Filter extends BaseFilterForm{
  ...
  protected readonly documentNumberField = '[class="datepicker-wrapper"] input[placeholder = "Document Number"]';
  protected readonly dateField = '[class="datepicker-wrapper"] input[placeholder = "Date"]';
...
class TransactionsTable extends BaseTransactionsTable {
  ...
  openCard(documentNumber: string) : TransactionCard {
    ...
    cy.wait('@transactionData').its('response.statusCode').should('equal', 200, 'Check transaction data loaded');
...
class TransactionCard extends BaseTransactionCardForm {
  ...
  protected readonly transactionTypeField = 'span[data-bind="text": $document.type]';
  protected readonly documentNumberField = 'span[data-bind="text": $document.number]';
  protected readonly fareField = 'span[data-bind="text": $document.fare]';
  protected readonly creationDateField = 'span[data-bind="text": $document.date]';
...
class CorrectionForm {
  ...
  save() {
    ...
    cy.wait('@saveTransaction').its('response.statusCode').should('equal', 200, 'Check transaction save');
  }

...
describe('Page Object example spec', () => {
  ...
  const propFileTemplatePath = './cypress/fixtures/propFiles/templates/propFileTemplate.gge';
  ...
  before(() => {
    ...
    const [propSaleDataFile, propSaleDocumentNumber] = dataProvider.generatePropData(
    ...
    dataProvider.createFile(propSaleDataFile, propFileTemplatePath, 
      basePropFileName, documentNumbers.firstPropFiles.propSale);
    // Deleting data by generated documentNumber from test environment just in case
    ProjectApi.first.getFilteredTransactionsList(documentNumbers.firstPropFiles.propSale).then(items => {
